 ...
 const requiredPassword = 'as2025'; // Access password for restricted sections
 let passwordVerified = false; // Flag if password has been entered correctly
 ...
 const domElements = {
     ...
     // itemBarcodeInput: document.getElementById('item-barcode'), // Removed
     // Item List
     ...
     addItemBtn: document.getElementById('add-item-btn'),
      ...
      reportNetProfit: document.getElementById('report-net-profit'),
       ...
       dbLowStockItems: document.getElementById('db-low-stock-items'), // New ID
 };
...
// --- Utility Functions ---
     ...
     try {
          ...
          switch (sectionId) {
              ...
              case 'categories-section':
                  ...
                  break;
              case 'stock-section':
                   if (passwordVerified) { // Only load if password was entered for this session
                      await loadStockItems(); // Load stock table
                      await renderStockCategoryTabs(); // Load category tabs for stock filtering
                   } else {
                       // Section is hidden until password is correct, data shouldn't load
                       // unless passwordVerified is true. This case handles direct URL access attempt?
                   }
                  break;
              ...
              case 'daily-report-section':
                   if (passwordVerified) { // Only load if password was entered for this session
                      domElements.dailyReportDateInput.value = new Date().toISOString().split('T')[0]; // Set default date
                   }
                  break;
 ...
 function translateUI(lang) {
     ...
     document.querySelectorAll('[data-translate-key]').forEach(element => {
      ...
      if (document.getElementById('stock-section').classList.contains('active') && passwordVerified) loadStockItems(); // Only if visible
      if (document.getElementById('purchases-section').classList.contains('active')) loadPurchases();
 ...
 function handlePasswordSubmit() {
     ...
     if (enteredPassword === requiredPassword) {
         ...
         if (targetLink && sectionToAccess) {
             // Use requestAnimationFrame to ensure modal closes before navigating/loading
         ...
         updateElementText(domElements.passwordError, 'password_prompt_error');
          ...
          passwordVerified = false; // Ensure it's false on incorrect attempt
     }
 }
 ...
 async function executeDelete() {
     ...
     try {
          ...
          if (type === 'category') {
               ...
               if (document.getElementById('stock-section').classList.contains('active') && passwordVerified) await loadStockItems(); // Update Stock table
           } else if (type === 'item') {
  ...
  async function executeClearAllData() {
      ...
      try {
          ...
          showNotification(translateKey('alert_data_cleared'), 'success');
           ...
           // Re-add default settings (language) within a new transaction
           try {
               ...
               setTimeout(async () => {
                    ...
                    await settingsTx.objectStore('settings').put({ key: 'language', value: 'en' });
               ...
               console.error("Error restoring default settings:", settingsError);
 ...
 function setupEventListeners() {
     ...
     domElements.posSearchInput.addEventListener('input', () => {
          const activeCategoryTab = domElements.categoryTabs.querySelector('.category-tab.active');
          const categoryId = activeCategoryTab ? activeCategoryTab.dataset.categoryId : 'all';
          updateProductsGrid(categoryId, domElements.posSearchInput.value);
     });
